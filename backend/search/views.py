...
def global_search_view(request):
    ...
    if has_module_permission(request.user, 'PRODUCT', 'view'):
        product_qs = Product.objects.filter(is_active=True).filter(
            Q(name__icontains=q) | Q(code__icontains=q)
        )[:5]

    case_qs = Case.objects.none()
    if has_module_permission(request.user, 'CASE', 'view'):
        case_qs = Case.objects.filter(title__icontains=q)[:5]

    doc_qs = Document.objects.none()
    if has_module_permission(request.user, 'DOCUMENT', 'view'):
        doc_qs = Document.objects.filter(name__icontains=q)[:5]

    quote_qs = Quote.objects.none()
    if has_module_permission(request.user, 'QUOTE', 'view'):
        quote_qs = Quote.objects.filter(Q(title__icontains=q) | Q(customer_name__icontains=q))
        if not request.user.is_admin:
            quote_qs = quote_qs.filter(
    ...
    return Response({
        ...
        'quotes': [{'id': item.id, 'title': item.title, 'customer_name': item.customer_name}
                   for item in quote_qs],
    })
